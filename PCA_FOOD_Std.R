...
data <- read.csv(file = "C:/Business Analytics/Winter/Multivariate/Week3/food.csv")
data
...
sigma=cor(data) ## standardisation of data.
sigma
vars=diag(sigma)
...
## C: sum of variances  
var1+var2+var3+var4+var5
v= var(y1)+var(y2)+var(y3)+var(y4)+var(y5)
...
ts.plot((percentvars_pc),col=c("red"),xlab="ith vector",ylab="percent variance")
...
set.seed(1002)
dv=rowSums(data)+rnorm(24,mean=0,sd=10) #24 is beacuse we have 24 cities
